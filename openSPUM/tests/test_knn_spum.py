"""SPUM with KNN surface connections instead of chains."""
...
for i in range(12):
    ...
    if i > 0:
        p.add_connection(1+i, 1+(i-1))  # ring connections
p.add_connection(1, 12)  # close the ring

# Add more surface particles via step1b over frames
sigmas = []
for frame in range(1, 61):
    ...
    if frame <= 30 or frame % 10 == 0:
        print(f"F{frame:>2}: V={p.active_count():>3} c={c1+c1b:>2} e={c2:>3} ov={c3b:>3} d={dc:>2} p={c5:>2} re={rn:>2} Σ={s:>4} {dt:.2f}s", flush=True)
    if p.active_count() < 3:
