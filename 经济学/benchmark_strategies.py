...
def rsi(values: list[float], window: int = 14) -> list[float]:
    ...
    rsi_vals = [float('nan')]  # 第一个值没有 delta
    avg_gain = 0.0
    avg_loss = 0.0
...
def ma_crossover_signal(prices: list[float], fast: int = 5, slow: int = 20) -> list[float]:
    ...
    for i in range(len(prices)):
        if math.isnan(sma_fast[i]) or math.isnan(sma_slow[i]) or prices[i] == 0:
            ...
            signals.append(max(-1.0, min(1.0, diff * 10)))  # 放大系数 10
    return signals


def rsi_signal(prices: list[float], window: int = 14,
               oversold: float = 30, overbought: float = 70) -> list[float]:
    """
    RSI 超买超卖信号。
    ...
    for r in rsi_vals:
        if math.isnan(r):
            ...
            signals.append(1.0)  # 超卖 → 做多
        elif r > overbought:
            signals.append(-1.0)  # 超买 → 做空
        else:
            # 中间区线性过渡
...
def macd_signal(prices: list[float], fast: int = 12, slow: int = 26,
                signal_period: int = 9) -> list[float]:
    """
    MACD 信号。
    ...
    for i in range(len(prices)):
        if math.isnan(histogram[i]) or prices[i] == 0:
            ...
            s = histogram[i] / prices[i] * 50  # 归一化系数
...
def bollinger_signal(prices: list[float], window: int = 20, num_std: float = 2.0) -> list[float]:
    ...
    for i in range(len(prices)):
        if math.isnan(upper[i]) or middle[i] == 0:
            ...
            signals.append(-1.0)  # 突破上轨 → 做空
        elif prices[i] <= lower[i]:
            signals.append(1.0)   # 跌破下轨 → 做多
        else:
            # 在带内线性衰减
...
def compute_all_benchmarks(dates: list[str], prices: list[float]) -> dict:
    ...
    return {
        ...
        '布林带(20,2σ)': bollinger_signal(prices, 20, 2.0),
