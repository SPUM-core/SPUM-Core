...
# File: backtest_engine.cpython-311.pyc (Python 3.11)
...
class StrategyResult:
    ...
    ic_metrics: dict = field(default_factory=dict)
...
def compute_ic_series(predictions: list[float], actuals: list[float], window: int = 20) -> list[float]:
    ...
    for i in range(window, n):
        ...
        den = math.sqrt(sum((rp[j] - rp_m) ** 2 for j in range(window)) * sum((ra[j] - ra_m) ** 2 for j in range(window))) + 1e-10
...
class BacktestEngine:
    ...
    def __init__(self, initial_capital: float = 1000000, commission_rate: float = 0.0003, slippage: float = 0.001):
        self.initial_capital = initial_capital
        self.commission_rate = commission_rate
        self.slippage = slippage

    
    def run(self, dates: list[str], signals: list[float], prices: list[float], benchmark_returns: Optional[list[float]] = None, strategy_name: str = 'Strategy') -> StrategyResult:
        '''
        运行单策略回测。
        ...
        if n < 20:
            return StrategyResult(name = strategy_name, daily_returns = [], cumulative_returns = [], signals = [], positions = [], trades = [], metrics = {})
        dates = dates[:n]
        signals = signals[:n]
        prices = prices[:n]
        ...
        for i in range(1, n):
            ...
            if abs(delta_pos) > 0.001:
                cost = abs(delta_pos) * (self.commission_rate + self.slippage)
            else:
                cost = 0.0
            ...
            if abs(delta_pos) > 0.02 and target_pos != position:
                trade_log.append({
                    ...
                    'delta': round(delta_pos, 3),
                    'pos_after': round(target_pos, 3),
                    'cost': round(cost * 100, 3)})
            position = target_pos
        metrics = compute_portfolio_metrics(daily_returns)
        return StrategyResult(name = strategy_name, daily_returns = daily_returns, cumulative_returns = cumulative_returns, signals = signals, positions = [max(-1, min(1, s)) for s in signals], trades = trade_log, metrics = metrics)
...
def compare_strategies(results: list[StrategyResult]) -> str:
    ...
    lines = []
    lines.append('==========================================================================================')
    lines.append('SPUM 五形回测框架 — 多策略对比')
    ...
    for r in results:
        if r.trades:
            ...
            for t in r.trades[:10]:
                lines.append(f'''  {t['date']} {t['direction']:<5} Δ={t['delta']:<+7.3f} 仓位={t['pos_after']:<+7.3f} 成本={t['cost']:.3f}%''')
            if len(r.trades) > 10:
...
def compute_cross_sectional_ic(pairs: list[dict]) -> dict:
    ...
    den = math.sqrt(sum((rp[i] - rp_m) ** 2 for i in range(n)) * sum((ra[i] - ra_m) ** 2 for i in range(n))) + 1e-10
    ic = num / den
    ...
    for label, pairs_list in by_label.items():
        ...
        n_den = math.sqrt(sum((rpp[j] - rpp_m) ** 2 for j in range(nn)) * sum((raa[j] - raa_m) ** 2 for j in range(nn))) + 1e-10
    ...
    return {
        ...
        'ic_ir': round(float(ic / (np_std(list(label_ics.values())) + 1e-10)) if label_ics else 0, 4),
        'n': len(valid),
        'by_label': label_ics}
...
if __name__ == '__main__':
    ...
    print(f'''随机信号    Sharpe={r1.metrics.get('sharpe', 0):.3f} Return={r1.metrics.get('total_return', 0):.1f}%''')
    sig2 = []
    for i in range(n):
        if i < 5:
            sig2.append(0)
    ...
    print(f'''5日动量     Sharpe={r2.metrics.get('sharpe', 0):.3f} Return={r2.metrics.get('total_return', 0):.1f}%''')
