...
| `network/module_nodes.txt` | 新增 | **模块节点统一注册表** — 集中管理 GT/SOC/ECON/LING/FI/COSM/COG/GEO 等模块节点（几何学 GEO-001~038 已注册） |
| `network/edges.txt` | 33+15+11+7+21边 | 核心推导链 + 模块桥接边 + 实验验证边（EX-001 温差力矩 / EX-002 拓扑认知）+ COG 元认识论桥接边 + GEO 几何学投影层桥接边 |
...
| 文件 | 内容 |
|------|------|
| `docs/SPUM2610/` 已删除（冗余副本，2026-08-16 清理） | 完整理论正文以根目录 `SPUM2610.md` 为唯一权威 |
...
| 文件 | 内容 |
|------|------|
| `.trae/skills/zhihu-knowledge/skill.md` | 知识库索引 Skill：19 篇已收录文章 + 加载策略 + 写作风格指南 |
...
> 9 个子目录，24+ 概念文件，1 个层级 Skill。每个物理学名词还原为 ⟨P, ε⟩ 的拓扑响应。详见 [总索引](../../物理学/README.md) 与 [Skill](../../物理学/skill.md)。

| 子目录 | 文件数 | 核心内容 |
|--------|-------|---------|
| `物理学/基本相互作用/` | 5 | 强力=拓扑闭合禁锢、电磁力=层级可编织性差异、弱力=守恒律松弛、引力=空间流（读数 ∇σ）、四力统一 |
...
> 1 个层级 Skill，26 节点。SPUM-图论不是经典图论的"一个分支"——是在不同本体论预设下重建的独立图论体系。**连接优先于属性**（孤立节点不存在）、**悬挂端不可消除**（演化引擎）、**图状态是帧快照**（无全局图）、**完美不达**（完全图不可实例化）、**局部闭合充分**（暂态矛盾允许）。所有应用模块（社会学/经济学/语言学/物理学）的 deg, σ, δ, V⁺, V⁻ 运算均以此为统一基底。v2.2 注册**几何图论层 GT-027~032**（摆动角 Δθ≈28.72°、键角互补 151.28°、配位上限 12、壳层堆叠角 14.359°、缺口方向 n̂）——拓扑合法性 ≠ 角度自由度。详见 [总索引](../../图论/README.md) 与 [Skill](../../图论/skill.md)。
...
| `spum-几何图论.md` | **几何图论层 GT-027~032**：五公设 G1~G5（球体存在/不可相入/相切即边/缺口方向/角度自由度）+ 五定理 T1~T5（摆动角 28.7185°、键角互补 151.28°、配位上限 12、壳层堆叠角 14.359°、缺口方向自由度）+ 键角推导——甲烷 109.47°=arccos(−1/3) 判决性测试（6节点） |
| `spum-计数关系.md` | **可审计计数关系层 C1~C9**：「对象→计数操作→数值→约束」四元组规范——Σ(6−deg)=12、N=12/(6−C)、V=12/E=30/F=20、实面12/边虚面30/面虚面20、候选定理 C_max=E+F=50、截断 42≤50<62、60° 张角判据；每条携带验证路径（9节点）+ 待证项 T1~T4 |
| `spum-图论节点.md` | 26 节点集中注册表 + 几何图论层 GT-027~032 完整定义（6节点） |

## 五-15、SPUM 几何学——投影层几何（`几何学/`）

> 1 个层级 Skill（技能 ID `spum-geometry`），38 节点（GEO-001~038），8 文件。核心：**几何非本体**——几何概念是 ⟨P, ε⟩ 关系网络在认知投影中必然涌现的产物，不是宇宙的先验结构。分层：公理层（GEO-001~008）/ 欧氏发生学层（009~017）/ 组合恒等式层（018~020）/ Finsler 公设层（021~026）/ Randers 度量层（027~030）/ 矛盾二解答层（031~033）/ 应用桥层（034~038）。**分层边界**：GEO-021~033 属 L2 投影层，与 `图论/` GT-027~032（L3 球体几何，直接本体）不可互推。节点注册见 `network/module_nodes.txt`，桥接边见 `network/edges.txt`。详见 [总索引](../../几何学/README.md) 与 [Skill](../../几何学/skill.md)。

| 文件 | 核心命题 |
|------|---------|
| `spum-几何公理.md` | **公理层 GEO-001~008**：几何投影公理 / 关系先于几何 / 离散优先 / π 降级 / 维度发生学 / 球体逻辑必然性 / 投影不变式三分（R/P/C）/ 五层几何角色 |
| `欧氏几何/spum-发生学.md` | **GEO-009~017**：关系→节点→点→线→面→体→维度→距离→π 九步发生链 |
| `欧氏几何/spum-组合恒等式.md` | **GEO-018~020**：球面欧拉恒等式 V−E+F=2 / 角度亏损 Σ(6−deg)=12 / 高斯-博内投影 4π=12·(π/3) |
| `Finsler几何/spum-Finsler公设.md` | **GEO-021~026**：F1 方向依赖 / F2 不可逆度量（**推论 R1 往返盲定理**：F(v)+F(−v)=2√g ⇒ b 在 MM 族往返/梯度场环路中精确相消，与 ∇σ 大小无关） / F3 σ 场各向异性 / F4 V⁺V⁻ 对偶 / F5 诱导坐标 / F6 Randers 形式 |
| `Finsler几何/spum-Randers度量.md` | **GEO-027~030**：F=√g+b，g 编码 σ、b 编码 ∇σ / 引力幂次退化 / 红移累积读数（推论 R1 往返盲定理见 §1.4；红移是光子数据链的聚合读数 z ∝ Σ(σ−σ₀)/σ₀，两端 σ 之比为退化读数） |
| `Finsler几何/spum-矛盾二解答.md` | **GEO-031~033**：嵌入度量定义——Finsler 是 L0.5→L2 的正确投影算子；矛盾二"嵌入度量未定义"的建议改判（⏸️→✅，**待裁定**） |
| `spum-几何应用桥.md` | **GEO-034~038**：物理/宇宙学/化学/图论→几何 / L0-L4 统一 |

## 五-16、实体实验验证（`温差力矩实验/` + `experiments/`）

> SPUM 的两个实验验证支柱：**温差力矩实验**（桌面物理实验：Peltier温差扭摆天平，不对称空间抽运→净力矩的竞力反演）和 **拓扑认知实验群**（深度学习实验：悬挂端梯度范数、锚点漂移，P0/P1/P2/N015/N016 嵌套验证）。

| 实验 | 文件 | 核心结果 |
|------|------|---------|
| 温差力矩 | `温差力矩实验/README.md` | 保留 4-26/4-27 两日数据 + 25页报告，3 数据集竞力拟合。最佳数据 f₀=0.997Hz, ζ=0.0964, R_type≈2.5 跨条件一致。阻尼谐振子竞力模型验证了温差力矩=温差驱动项与惯性恢复项的竞争系统 |
...
| SPUM 几何学 | `AGENT.md` + `spum-core` + `几何学/skill.md` + `几何学/README.md` + 对应 spum-几何*.md / Finsler几何/*.md 文件 |
