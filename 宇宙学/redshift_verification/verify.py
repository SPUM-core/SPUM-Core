...
class RedshiftVerifier:
    ...
    def run(self, cluster_names: Optional[List[str]] = None,
        ...
        """
        print("=" * 60)
        print("SPUM 红移验证 — 簇内局域退化读数 z = σ_source/σ_obs - 1")
        print("=" * 60)
    ...
    def _redshift_derivation_chain(self) -> str:
        ...
        chain = [
            ...
            "│",
            "├─ [定义] 红移 z 是光子数据链的累积读数（**局域两端退化形式**）",
            "│   ├─ 通式: z ∝ Σ_k (σ(l_k)−σ₀)/σ₀  ← 光子途中不被改造，逐跳追加 σ 记录",
            "│   ├─ 局域两端近似（本验证包适用）: z = f(σ_obs)/f(σ_source) - 1",
            "│   ├─ 一阶近似: f(σ) ∝ 1/σ → z = σ_source/σ_obs - 1",
            "│   └─ 观测 = 两个星系子网的拓扑耦合读数（**不是**单个光子被拉长）",
            "│",
            "├─ [推演] 同一星系团内:",
